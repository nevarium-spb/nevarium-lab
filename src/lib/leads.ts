...
// Адрес CRM также захардкожен в connect-src заголовка Content-Security-Policy
// (vercel.json) — если VITE_CRM_API когда-нибудь укажет на другой домен,
// CSP заблокирует fetch к нему как чужой источник, и заявки перестанут
// уходить молча (запрос упадёт по CSP, а не по таймауту). Менять оба места
// вместе.
